...
vars_cluster <- BODYDATA[, c("waist_girth","chest_girth","hip_girth","shoulder_girth","thigh_girth")]

# Normalizar datos
vars_scaled <- scale(vars_cluster)

# K-means clustering (k=3 por ejemplo)
set.seed(123)
km <- kmeans(vars_scaled, centers = 3, nstart = 25)

# Añadimos la etiqueta al dataset
BODYDATA$cluster <- factor(km$cluster)

# Representación gráfica
ggplot(BODYDATA, aes(x=waist_girth, y=hip_girth, color=cluster)) +
  geom_point(alpha=0.7) +
  labs(title="Clusters de perfiles corporales",
       x="Waist Girth", y="Hip Girth") 
```
